...
/**
 * 시나리오를 C# Microsoft.Playwright 코드로 자동 컴파일/생성
 */
export function generatePlaywrightCSharpCode(scenario) {
  if (!scenario) return '';
  const lines = [
    '// ── Microsoft.Playwright C# (.NET 8/9) 자동 생성 스크립트 ──',
    'using System;',
    ...
    '{',
    '    public class AutoGeneratedScenario',
    '    {',
    ...
    '                ViewportSize = ViewportSizeIfMacro()',
    '            });',
    '            ',
  ...
  (scenario.steps || []).forEach(step => {
    ...
    if (step.action === 'NAVIGATE') {
      ...
      if (step.sendEnter) {
        lines.push(`            await page.Locator("${step.selector || ''}").PressAsync("Enter");`);
      }
  ...
  lines.push('            Console.WriteLine("Playwright RPA 시나리오 정상 완료");');
  lines.push('        }');
  lines.push('    }');
  lines.push('}');
  return lines.join('\n');
