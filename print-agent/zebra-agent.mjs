/**
 * ============================================================
 * zebra-agent.mjs  --  Zebra GK-420D PC Local Print Agent v1.1
 * ============================================================
 * 시작 옵션:
 *   node zebra-agent.mjs           -- 저장된 설정으로 즉시 시작
 *   node zebra-agent.mjs --setup   -- 대화형 프린터 재설정 강제 실행
 *
 * 설정 저장: ./agent-config.json (자동 생성/갱신)
 * ============================================================
 */
...
const SUPABASE_URL = process.env.SUPABASE_URL
  || 'https://tfgbpgutxxlhqbzewkyt.supabase.co';
const SUPABASE_KEY = process.env.SUPABASE_KEY
  || 'sb_publishable_wruJQfp3Op-ISvVwb4ZdmA_2OqMUJeQ';
...
function saveConfig(cfg) {
  ...
  log('SETUP', '설정 저장 완료: ' + CONFIG_PATH);
}
...
// 포트명에서 TCP 호스트 추출:  IP_192.168.1.50 → 192.168.1.50 / USB,WSD → 127.0.0.1
...
// 저장된 설정 확인 화면 (5초 타임아웃, R 누르면 재설정)
async function showConfigAndConfirm(config) {
  ...
  console.log('  주소    : ' + config.printerHost + ':' + config.printerPort);
...
function checkPrinterConnection(host, port) {
  return new Promise(resolve => {
    const s = net.createConnection({ host, port }, () => { s.end(); log('INFO', '[OK] 프린터 연결 확인 (' + host + ':' + port + ')'); resolve(true); });
    s.on('error', err => { log('WARN', '프린터 연결 불가 - ' + err.message); log('WARN', '프린터 전원 및 연결을 확인하세요. 에이전트는 계속 실행됩니다.'); resolve(false); });
    s.setTimeout(3000, () => { s.destroy(); log('WARN', '프린터 연결 타임아웃 (3초)'); resolve(false); });
  });
}
...
async function processQueueItem(row, supabase, config) {
  ...
  try {
    ...
    await supabase.from('print_queue').update({ print_status: 'PRINTED', printed_at: new Date().toISOString() }).eq('id', id);
  } catch (err) {
    log('ERR', 'ZPL 전송 실패', err.message);
    await supabase.from('print_queue').update({ print_status: 'ERROR', print_error: err.message }).eq('id', id);
  }
}
...
function setupRealtimeSubscription(supabase, config) {
  ...
  const ch = supabase.channel('zebra-print-agent')
    ...
    .subscribe((status, err) => {
      if (status === 'SUBSCRIBED') log('INFO', '[OK] Realtime 구독 완료 (zebra-print-agent)');
...
async function main() {
  console.log('');
  console.log('+=================================================+');
  console.log('|  Zebra GK-420D PC Print Agent  v1.1            |');
  ...
  log('INFO', '라벨 규격 : ' + config.labelWidthMm + 'mm x ' + config.labelHeightMm + 'mm / Code39');
  console.log('');
  ...
  console.log('  재설정 : node zebra-agent.mjs --setup');
  console.log('  종료   : Ctrl + C');
  console.log('');
}

main().catch(err => { log('ERR', '초기화 실패', err.message); process.exit(1); });
