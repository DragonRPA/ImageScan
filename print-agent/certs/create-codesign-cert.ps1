...
# DragonRPA 사내 코드 서명 인증서 자동 생성 및 로컬 등록

$ErrorActionPreference = "Stop"

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Definition
if (-not $scriptDir) { $scriptDir = (Get-Location).Path }
$pfxPath   = Join-Path $scriptDir "DragonRPA_CodeSign.pfx"
$cerPath   = Join-Path $scriptDir "DragonRPA_Root.cer"
$pfxPass   = "DragonRPA@2026"
$secPass   = ConvertTo-SecureString -String $pfxPass -Force -AsPlainText

Write-Host "========================================================" -ForegroundColor Cyan
Write-Host "  DragonRPA 사내 코드 서명 인증서 생성 및 로컬 등록 시작" -ForegroundColor Cyan
Write-Host "========================================================" -ForegroundColor Cyan
...
$cert = New-SelfSignedCertificate `
    ...
    -Subject "CN=DragonRPA Software, O=DragonRPA, OU=System RPA, C=KR" `
...
# dist 폴더로 CER 복사
$targetDist = Join-Path (Split-Path -Parent $scriptDir) "dist"
if (Test-Path $targetDist) {
    Copy-Item -Path $cerPath -Destination (Join-Path $targetDist "DragonRPA_Root.cer") -Force
    Write-Host " -> dist/DragonRPA_Root.cer 복사 완료" -ForegroundColor Green
}

# public 웹 루트로도 CER 복사
$targetPublic = Join-Path (Split-Path -Parent (Split-Path -Parent $scriptDir)) "public"
if (Test-Path $targetPublic) {
    Copy-Item -Path $cerPath -Destination (Join-Path $targetPublic "DragonRPA_Root.cer") -Force
    Write-Host " -> public/DragonRPA_Root.cer 복사 완료" -ForegroundColor Green
}
