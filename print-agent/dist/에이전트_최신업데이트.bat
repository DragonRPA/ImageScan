...
:: 3. GitHub에서 최신 바이너리 안전 다운로드 (PowerShell BITS / WebClient)
echo [2/4] GitHub 최신 배포 바이너리 다운로드 중...
...
:: 4. Windows Defender / SmartScreen 보안 차단 해제 (Unblock-File)
echo [3/4] Windows 보안 차단 플래그 자동 해제 (Unblock-File)...
powershell -NoProfile -Command "Unblock-File -Path '%~dp0temp\agent-update.exe' -ErrorAction SilentlyContinue"

:: 5. 파일 교체
echo [4/4] 최신 바이너리 교체 및 자동 재실행...
